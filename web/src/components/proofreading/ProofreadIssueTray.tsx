...
const normalizeSeverity = (value?: string | null) => {
  ...
  if (normalized.includes('critical') || normalized === '4' || normalized === '5') {
    return 'critical';
  }
...
export const ProofreadIssueTray = () => {
  ...
  const grouped = useMemo(() => {
    return segments
      ...
      .filter((group): group is { segment: (typeof segments)[number]; entries: (typeof issues)[number][] } => Boolean(group));
  ...
  if (!grouped.length) {
    return (
      <div className="flex h-full min-h-0 flex-col overflow-hidden rounded border border-slate-200 bg-white p-4 text-xs text-slate-500">
        ...
        <p className="mt-2 text-slate-500">No issues found for this dataset yet.</p>
      </div>
    );
  }
  ...
  return (
    <div className="flex h-full min-h-0 flex-col overflow-hidden rounded border border-slate-200 bg-white">
      <header className="border-b border-slate-200 px-4 py-3">
        <h3 className="text-sm font-semibold text-slate-700">Issue Tray</h3>
        <p className="text-xs text-slate-500">Select an issue to focus the editor.</p>
      </header>
              ...
              <ul className="divide-y divide-slate-100">
                {entries.map((issue, index) => {
                  ...
                  const recommendation =
                    ...
                    null;
                  const statusValue = String(issue.status ?? 'pending').toLowerCase();
                  const statusClass =
                    statusBadgeClasses[statusValue] ?? statusBadgeClasses.default;
                  const timestamp = formatTimestamp(issue.updatedAt ?? issue.createdAt);
                  const isActive = activeIssueId === issue.id;
                  const listKey = issue.id
                    ? `${issue.id}-${index}`
                    : `${issue.bucket?.subfeatureKey ?? issue.bucket?.group ?? 'issue'}-${segment.segmentId}-${index}`;
                  const guardNotes = (issue.notes as ProofreadingIssue['notes'] | undefined)?.guardFindings;
                    ...
                    <li key={listKey}>
                      ...
                      >
                        <div className="flex items-center justify-between gap-2">
                          ...
                          <span className={`inline-flex items-center rounded-full px-2 py-0.5 text-[11px] font-semibold ${severityClass}`}>
                        ...
                        {recommendation && (
                          <p className="text-xs text-slate-500 line-clamp-3">{recommendation}</p>
                        )}
                      </button>
                    </li>
                  );
