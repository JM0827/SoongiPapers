...
const usageEventLabelMessages: Record<
  string,
...
const deriveProjectModalProfile = (
...
export const LeftSidebar = () => {
  ...
  const consentInfoToggleLabel = showConsentInfo
    ? consentInfoHide
    : consentInfoShow;
  ...
  const submitRename = useCallback(async () => {
    ...
    const previousUserConsent = parseUserConsent(
      modalState.project.user_consent,
    );
    ...
    try {
      ...
      showToast(
        localize("sidebar_toast_project_updated", "Project details updated."),
      );
      closeModal();
    } catch (error) {
        ...
        <label className="mt-4 block text-xs font-semibold tracking-wide text-slate-500">
          ...
          <input
            ...
            placeholder={localize(
              "sidebar_create_placeholder_title",
              "New project",
            )}
  ...
  const handleCompleteProject = useCallback(
    ...
    [token, invalidateProjects, invalidateProjectContent, localize],
  );
  ...
  >(
    () => ({
      ...
      translation: {
        key: "sidebar_activity_translation",
        fallback: "Translation",
      },
      proofreading: {
        key: "sidebar_activity_proofreading",
        fallback: "Proofreading",
      },
      quality: {
        key: "sidebar_activity_quality",
        fallback: "Quality review",
      },
      publishing: {
        key: "sidebar_activity_publishing",
        fallback: "Publishing",
      },
    }),
    [],
  ...
  const recentUpdates = useMemo(() => {
    ...
    return lines;
  }, [localize, snapshot, stageLabelMap]);
  ...
  const quickActions: QuickAction[] = [
    ...
    },
    {
      ...
      disabled:
        !chatExecutorReady ||
        translationRunning ||
        Boolean(translationGuardReason),
    ...
    },
    {
      ...
      disabled: !chatExecutorReady || !hasTranslation || proofreadingRunning,
      status: proofreadingRunning
        ...
        : proofreadingDone
          ...
          : "default",
      onClick: async () => {
        if (!chatExecutorReady || !hasTranslation || proofreadingRunning)
          return;
    ...
    },
    {
      ...
      disabled: !chatExecutorReady || !translationDone || qualityRunning,
      status: qualityRunning ? "running" : qualityDone ? "done" : "default",
      onClick: async () => {
    ...
    },
    {
      key: "view-proofread",
      label: localize(
        "sidebar_quick_view_proof_label",
          ...
          >
                    ...
                    <div className="mt-3 space-y-2">
                      <div>
                        ...
                        <ul className="mt-1 list-disc space-y-1 pl-4 text-slate-600">
                          {consentContent.needsConsentItems.map(
                            (item, index) => (
            ...
            <div className="flex flex-wrap items-center gap-4 text-xs font-semibold tracking-wide text-slate-500">
              <span className="flex items-center gap-1">
                <span>
                  {localize(
                    "sidebar_project_modal_origin_lang",
                    "Origin language",
                  )}
                </span>
              ...
              <span className="flex items-center gap-1">
                <span>
                  {localize(
                    "sidebar_project_modal_target_lang",
                    "Target language",
                  )}
                </span>
                <span className="font-normal text-slate-700">
            ...
            <div className="grid gap-4 md:grid-cols-2">
              <div className="space-y-4">
                ...
                <label className="flex flex-col gap-1 text-xs font-semibold tracking-wide text-slate-500">
                  {localize(
                    "project_profile_field_author_notes",
                    "Author notes",
                  )}
                  <textarea
              ...
              {localize(
                ...
                'Do you want to delete "{{title}}"?',
                {
                  title:
                    ...
                    localize("sidebar_project_untitled", "Untitled"),
                },
              )}
            </p>
            <div className="flex justify-end gap-2">
