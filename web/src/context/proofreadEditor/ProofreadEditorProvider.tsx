...
const reducer = (
  state: ProofreadEditorState,
  action: Action,
): ProofreadEditorState => {
  switch (action.type) {
    case "LOAD_SUCCESS": {
      const {
        dataset,
      ...
      const selectedSegmentId = mapped.orderedIds.includes(
...
const ProofreadEditorContext =
  createContext<ProofreadEditorContextValue | null>(null);
...
export const ProofreadEditorProvider = ({
  ...
  const savePendingChanges = useCallback(async () => {
    ...
    if (state.featureToggles.originOnly || state.featureToggles.readOnly)
      return;
        ...
        if (conflict.code === "CONFLICT") {
          ...
          if (!documentVersion) {
            try {
              ...
              console.warn(
                "[ProofreadEditorProvider] refetch after conflict failed",
                refetchError,
              );
            }
          }
          ...
          if (documentVersion) {
            try {
              ...
              console.warn(
                "[ProofreadEditorProvider] auto-retry after conflict failed",
                retryError,
              );
            }
          }
  ...
  const retryConflict = useCallback(async () => {
    ...
    try {
      const payload: ProofreadEditorPatchPayload = {
        translationFileId: state.dataset.translationFileId,
        documentVersion:
          state.conflict.documentVersion ??
