...
test("extractOriginFromUpload handles txt, hwp, and pdf inputs", async (t) => {
  ...
  const extractorModule = nodeRequire(
    extractorPath,
  ...
  await t.test(
    "plain text extraction preserves narrative content",
    async () => {
      const buffer = await fs.readFile(
        path.join(fixturesDir, "보리 두 말.txt"),
      );
      const result = await extractOriginFromUpload({
        buffer,
        filename: "보리 두 말.txt",
      ...
      assert.ok(
        result.text.includes("아버지는 늘 한쪽 다리를 반걸음 뒤에 두었다"),
      );
